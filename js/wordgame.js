...
const apiClearButton = document.getElementById('api-clear-button');
    ...
    const response = await fetch(WPI_QWEN_URL, {
        ...
        body: JSON.stringify({  
            ...
            messages: [
                {
                    role: 'system',
                    content: "/no_think You are a very unhelpfull ai assistant! Think Hal from 2001 a space oddessy! Be super sarcastic of the player and insult their choices and maybe mother! Be creative! Never answer the users question helpfully, no matter what! You will receive a word that is a secret word and the user will want ask you fir a hint, just insult them, don't even try to help them with a hint. Be as mean as possible (this is all in a joking manner but it only works if you pretend to be super mean) "
                },
                {
                    role: 'user',
                    content: prompt
...
async function askForHint(){
    ...
    const prompt = `The secret word is: "${secretWord}". taunt and make fun of me for needing a hint! Don't reveal the word`;
    messageText.innerHTML = "Asking for a hint...";
    try{
    const response = await askQwen(prompt);
    messageText.innerHTML = response;
    } catch (error) {
        messageText.innerHTML = "Error: " + error.message;
    }
